...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0D17] text-white font-sans selection:bg-[#00FF88] selection:text-black overflow-x-hidden">
      ...
      <button onClick={() => handleSimpleClick('Botão Flutuante')} className="fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#1ebc57] text-white p-4 rounded-full shadow-[0_0_30px_rgba(37,211,102,0.4)] transition-all flex items-center gap-3 border border-white/20 backdrop-blur-sm">
      ...
      <section className="relative min-h-[100vh] lg:min-h-[95vh] flex items-center pt-24 pb-12 overflow-hidden z-10">
        ...
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center relative z-10">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                ...
                <p className="text-gray-300 text-lg md:text-xl mb-8 leading-relaxed max-w-lg drop-shadow-md">Assista ao vídeo e entenda como transformar seu telhado em uma usina de dinheiro.</p>
                <div className="flex flex-col sm:flex-row gap-4">
                    <button onClick={() => handleSimpleClick('Botão Hero')} className="bg-[#00FF88] text-black font-extrabold py-4 px-8 rounded-full hover:bg-[#00e67a] shadow-[0_0_30px_rgba(0,255,136,0.4)] flex items-center justify-center gap-2">
                        <FaWhatsapp size={24}/> Falar com Consultor
                    </button>
                    <a href="#calculadora" className="bg-white/10 text-white font-bold py-4 px-8 rounded-full border border-white/10 flex items-center justify-center gap-2 backdrop-blur-sm">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }} className="relative group">
                 <div className="absolute -inset-1 bg-gradient-to-r from-[#00FF88] to-blue-600 rounded-2xl blur opacity-30"></div>
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
                <div className="grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        ...
                        <p className="text-gray-400 mb-8 leading-relaxed">Nossa inteligência artificial analisa seu consumo e projeta o sistema ideal.</p>
                    ...
                    <div className="bg-black/40 rounded-2xl p-6 border border-white/5 shadow-inner min-h-[460px] flex flex-col justify-center">
                        <AnimatePresence mode="wait">
                            {step === 1 && (
                                <motion.div key="step1" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                                    <label className="text-[#00FF88] text-xs font-bold uppercase mb-2 block">Valor Mensal da Conta</label>
                                    <input type="text" value={formData.valorConta} onChange={handleCurrencyInput} placeholder="R$ 0,00" className="w-full bg-white/5 border border-white/10 focus:border-[#00FF88] rounded-xl py-4 px-4 text-2xl font-bold text-white outline-none mb-6" />
                                    <label className="text-gray-400 text-xs font-bold uppercase mb-2 block">Tipo de Imóvel</label>
                                    <div className="grid grid-cols-4 gap-2 mb-6">
                                        {[{id:'residencial', icon:Home, label: 'Casa'}, {id:'comercial', icon:Building, label: 'Comércio'}, {id:'rural', icon:Tractor, label: 'Rural'}, {id:'industrial', icon:Factory, label: 'Indústria'}].map((tipo) => (
                                            <button key={tipo.id} onClick={() => setFormData({...formData, tipoImovel: tipo.id})} className={`p-3 rounded-xl border flex flex-col justify-center items-center gap-1 transition-all ${formData.tipoImovel === tipo.id ? "bg-[#00FF88] border-[#00FF88] text-black shadow-[0_0_15px_rgba(0,255,136,0.3)]" : "border-white/10 text-gray-400 hover:bg-white/10"}`}>
                                                <tipo.icon size={20} /><span className="text-[10px] font-bold uppercase">{tipo.label}</span>
                                            </button>
                                        ))}
                                    </div>
                                    <button onClick={handleCalculate} disabled={loadingSim} style={{ backgroundColor: '#00FF88' }} className="w-full text-black font-bold py-4 rounded-xl shadow-lg hover:brightness-110">{loadingSim ? "Processando..." : "Simular Economia"}</button>
                                </motion.div>
                            )}
                            {step === 2 && (
                                <motion.div key="step2" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} className="text-center">
                                    ...
                                    <button onClick={handleLeadSubmit} disabled={sendingLead} className="w-full bg-[#00FF88] text-black font-bold py-4 rounded-xl hover:bg-[#00e67a]">Ver Resultado</button>
                                </motion.div>
                            )}
                            {/* PASSO 3 - GRÁFICO BLINDADO ✅ */}
                            {step === 3 && (
                                <motion.div key="step3" initial={{scale:0.95, opacity:0}} animate={{scale:1, opacity:1}} className="text-center h-full flex flex-col justify-between">
                                    ...
                                    <div className="flex items-end justify-center gap-12 h-40 mb-10 px-4 w-full">
                                        <div className="w-14 flex flex-col justify-end items-center group relative h-full">
                                            ...
                                            <span className="absolute -top-8 text-red-500 font-bold text-xs group-hover:opacity-0 transition-opacity duration-300">Antes</span>
                                        ...
                                        <div className="w-14 flex flex-col justify-end items-center group relative h-full">
                                            ...
                                            <span className="absolute -top-8 text-[#00FF88] font-bold text-xs group-hover:opacity-0 transition-opacity duration-300">Velox</span>
                                            <motion.div initial={{ height: 0 }} animate={{ height: "10%" }} transition={{ duration: 1, delay: 0.5 }} className="w-full bg-[#00FF88] rounded-t-lg shadow-[0_0_20px_rgba(0,255,136,0.6)]"/>
                                        </div>
                                    </div>
                                    <div className="mb-6">
                                        <p className="text-gray-400 text-xs">Economia Anual Projetada</p>
                                        <div className="text-4xl font-black text-[#00FF88] mb-4">R$ {simulation.economiaAnual.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</div>
      ...
      <section className="py-24 px-6 relative z-10">
        <div className="container mx-auto max-w-6xl">
            <div className="text-center mb-16"><h2 className="text-3xl font-bold mb-4">Soluções Velox</h2><p className="text-gray-400">Tecnologia adaptada para cada necessidade.</p></div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {solutions.map((item, i) => (
                    <motion.div key={i} whileHover={{y:-5}} className="bg-white/5 backdrop-blur-md p-6 rounded-2xl border border-white/5 hover:border-[#00FF88]/30 transition-all group">
                        <div className="bg-white/5 w-12 h-12 rounded-lg flex items-center justify-center text-[#00FF88] mb-4 group-hover:scale-110"><item.icon size={24}/></div>
                    ...
                    <div key={i} className="border border-white/5 rounded-xl bg-white/5 overflow-hidden">
                        ...
                        <AnimatePresence>{openIndex === i && (
                            <motion.div initial={{height:0}} animate={{height:"auto"}} exit={{height:0}} className="overflow-hidden">
                                <div className="p-6 pt-0 text-gray-400 text-sm leading-relaxed border-t border-white/5">{faq.answer}</div>
                            </motion.div>
                        )}</AnimatePresence>
                    </div>
                ))}
                <div className="mt-4 p-6 bg-[#00FF88]/5 border border-[#00FF88]/20 rounded-2xl text-center">
                    <button onClick={() => handleSimpleClick('FAQ Lateral')} className="bg-[#00FF88] text-black px-8 py-4 rounded-full font-bold hover:bg-[#00e67a] flex items-center justify-center gap-2 mx-auto shadow-lg"><FaWhatsapp size={20} /> Falar com Especialista</button>
                </div>
             </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 grid md:grid-cols-3 gap-12 mb-12">
            ...
            <div className="text-left">
                ...
                <div className="flex gap-4">
                    ...
                    <button onClick={() => handleSimpleClick('Footer Zap')} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-[#00FF88] hover:text-black transition-all"><FaWhatsapp size={20}/></button>
                    <a href={emailLink} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-[#00FF88] hover:text-black transition-all"><FaEnvelope size={20}/></a>
                </div>
            </div>
        </div>
